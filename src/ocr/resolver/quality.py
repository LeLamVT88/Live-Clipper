"""Strict quality gate for resolved lineup segments."""
...
class QualityResult:
    """Result of validating one segment after lineup resolution."""
...
def evaluate_segment_quality(
    ...
    """Validate one segment before its result becomes final."""
    ...
    for lineup_index, lineup_records in sorted(groups.items()):
        if len(lineup_records) != expected_players:
            reasons.append(
                f"lineup {lineup_index} has {len(lineup_records)}/"
        ...
        if len(shirt_numbers) != len(lineup_records):
            reasons.append(
                f"lineup {lineup_index} has missing shirt numbers"
            )
        elif len(set(shirt_numbers)) != expected_players:
            reasons.append(
                f"lineup {lineup_index} has "
                f"{len(set(shirt_numbers))}/{expected_players} "
                "unique shirt numbers"
            )
        ...
        for record in lineup_records:
            ...
            try:
                ...
                pair_confidence = -1.0
            if (
                not math.isfinite(pair_confidence)
                or pair_confidence < min_pair_confidence
            ):
        ...
        if empty_names:
            reasons.append(
                f"lineup {lineup_index} has {empty_names} empty player name(s)"
            )
        if ui_names:
            reasons.append(
                f"lineup {lineup_index} contains UI text as player name: "
                + ", ".join(sorted(set(ui_names)))
            )
        if low_confidence:
            reasons.append(
                f"lineup {lineup_index} has {len(low_confidence)} "
                f"pair(s) below confidence {min_pair_confidence:.2f}"
            )
    ...
    return QualityResult(
        ...
        resolved_players=resolved_players,
        message=(
            f"quality gate passed for {len(groups)} lineup(s), "
            f"{resolved_players} player rows"
        ),
    )
