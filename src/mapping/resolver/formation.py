...
def signature_similarity(
    left: list[pd.Series], right: list[pd.Series],
    x_tolerance: float = 0.045, y_tolerance: float = 0.065,
) -> float:
...
def detect_formation_events(
    segment: pd.DataFrame, min_number_count: int,
    max_gap_seconds: float, signature_threshold: float,
) -> list[FormationEvent]:
    ...
    for frame_index, timestamp, signature in snapshot_candidates:
        ...
        best_similarity = max(
            signature_similarity(signature, existing) for existing in current.signatures
        )
...
def cluster_number_positions(
    event: FormationEvent, segment: pd.DataFrame,
    x_tolerance: float = 0.045, y_tolerance: float = 0.065,
) -> list[NumberCluster]:
...
def nearby_formation_labels(
    cluster: NumberCluster, event: FormationEvent, segment: pd.DataFrame,
    ...
    for frame_index, frame in text_rows.groupby("frame_index", sort=False):
        ...
        for _, row in frame.iterrows():
            ...
            # Paddle often places a tight number/name pair only 2–3% of the
            # frame height apart.  Keep this consistent with the anchor pass;
            # otherwise an accepted anchor can lose its label during the
            # final cluster resolution.
            if (
                dx <= 0.07
                and FORMATION_LABEL_MIN_GAP <= dy <= FORMATION_LABEL_MAX_GAP
            ):
...
def select_player_clusters(
    clusters: list[NumberCluster], event: FormationEvent,
    segment: pd.DataFrame, expected_players: int,
) -> list[NumberCluster]:
    for cluster in clusters:
    ...
    return sorted(ranked[:expected_players], key=lambda item: (item.center_y, item.center_x))
...
def validate_unique_shirt_numbers(records: list[dict[str, object]], lineup_index: int) -> None:
    ...
    if duplicates:
        ...
        raise LineupResolutionError(
            f"Duplicate shirt number(s) {duplicate_text} " f"found for lineup {lineup_index}."
        )


def resolve_event(
    event: FormationEvent, segment: pd.DataFrame, event_end_seconds: float,
    lineup_index: int, expected_players: int,
) -> list[dict[str, object]]:
    clusters = cluster_number_positions(event, segment)
    selected = select_player_clusters(clusters, event, segment, expected_players)
    ...
    selected.sort(key=lambda item: (row_indices[id(item)], item.center_x))
    label_results = {
        id(cluster): consensus_text(cluster.label_observations) for cluster in selected
    }
    missing_labels = [cluster for cluster in selected if not label_results[id(cluster)][0]]
    if missing_labels:
        raise LineupResolutionError(
    ...
    for slot_index, cluster in enumerate(selected, start=1):
        ...
        pair_confidence = (number_confidence * label_confidence * max(name_confidence, 0.01)) ** (
            1 / 3
        )
        records.append(
            {
                "video": str(segment["video"].iloc[0]), "segment_index": int(segment["segment_index"].iloc[0]),
                "lineup_index": lineup_index, "resolution_method": "formation",
                ...
                "slot_center_x_norm": round(cluster.center_x, 6), "slot_center_y_norm": round(cluster.center_y, 6),
            }
        )
    validate_unique_shirt_numbers(records, lineup_index)
    return records
...
def attempt_formation_resolution(
    ...
    max_gap_seconds: float, signature_threshold: float,
) -> tuple[pd.DataFrame, list[FormationEvent], list[dict[str, object]], list[str]]:
    ...
    for index, event in enumerate(events):
        event_end = (
            events[index + 1].start_seconds if index + 1 < len(events) else segment_end + 1e-6
        )
        resolved_lineup_index = len(segment_records) // expected_players + 1
        try:
