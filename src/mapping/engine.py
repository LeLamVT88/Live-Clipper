...
def write_csv(rows: list[dict[str, object]], output_csv: Path, columns: list[str]) -> None:
    output_csv.parent.mkdir(parents=True, exist_ok=True)
    pd.DataFrame(rows, columns=columns).to_csv(output_csv, index=False)


def classify_text(text: str) -> str:
    return "shirt_number_candidate" if text.isdigit() and 1 <= int(text) <= 99 else "text"
...
def create_ocr(cache_dir: Path, recognition_batch_size: int):
    ...
    try:
        ...
        raise LineupOCRError(
            "PaddleOCR is not installed. Run this script with `.venv-ocr/bin/python`."
        ) from exc
    return PaddleOCR(
...
def result_data(
    result: object, error_type: type[Exception] = LineupOCRError, source: str = "PaddleOCR",
) -> dict[str, object]:
    payload = getattr(result, "json", None)
    if callable(payload):
        payload = payload()
    if not isinstance(payload, dict):
        raise error_type(f"{source} returned an unsupported result format.")
    data = payload.get("res", payload)
    if not isinstance(data, dict):
        raise error_type(f"{source} result does not contain a result object.")
    return data


def detections_from_result(
    frame_record: dict[str, object], result: object, min_score: float,
) -> list[dict[str, object]]:
    data = result_data(result)
    texts, scores, boxes = (data.get(name, []) for name in ("rec_texts", "rec_scores", "rec_boxes"))
    ...
    for text, score, raw_box in zip(texts, scores, boxes, strict=True):
        ...
        try:
            ...
            raise LineupOCRError(f"Unsupported OCR box: {raw_box}") from exc
        if len(box) != 4:
        ...
        detections.append(
            {
                **frame_record,
                "text": text, "text_type": classify_text(text), "score": round(score, 6),
                "x1": x1, "y1": y1, "x2": x2, "y2": y2,
                "center_x": round(center_x, 3), "center_y": round(center_y, 3),
...
def run_ocr(
    frame_records: list[dict[str, object]], cache_dir: Path, output_csv: Path | None,
    min_score: float, batch_size: int, ocr: object | None = None, progress_label: str = "OCR",
) -> list[dict[str, object]]:
    if ocr is None:
    ...
    for start in range(0, len(frame_records), batch_size):
        ...
        input_paths = [str(resolve_project_path(Path(str(row["frame_path"])))) for row in batch]
        results = list(ocr.predict(input_paths))
        if len(results) != len(batch):
            raise LineupOCRError(
                f"PaddleOCR returned {len(results)} result(s) for {len(batch)} input frame(s)."
            )
        for frame_record, result in zip(batch, results, strict=True):
            detections.extend(detections_from_result(frame_record, result, min_score))
        completed = min(start + len(batch), len(frame_records))
        if output_csv is not None:
            write_csv(detections, output_csv, DETECTION_COLUMNS)
        print(f"{progress_label}: {completed}/{len(frame_records)} frames, {len(detections)} detections")
    return detections
