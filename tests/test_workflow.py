...
import ocr.workflow as workflow
    ...
    def test_workflow_runs_three_seven_then_full_segment(self) -> None:
            ...
            return [
                SegmentSelection(
                    ...
                    selected_frame_indices=tuple(
                        range(1, selected_frame_count + 1)
                    ),
                    message="selected",
                )
            ]
        ...
        def materialize(
            ...
            return [
                ...
                if int(record["frame_index"]) in selected
            ]
        ...
        def attempt(**kwargs: object) -> AttemptOutcome:
            ...
            return AttemptOutcome(
                ...
                detections={key: []},
                resolved_records={
                    key: resolved_players() if passed else []
                },
                diagnostics={
                    key: {
                        "video": key[0],
        ...
        config = PipelineConfig(
            segments_csv=temporary / "segments.csv",
            video_dir=temporary,
        ...
        self.assertEqual(
            [
                len(call.kwargs["target_records"])
