...
import mapping.resolver.formation as formation_resolver
import mapping.resolver.local_models as local_ocr_resolver
from mapping.resolver.common import NumberCluster
from mapping.resolver.formation import nearby_formation_labels
...
class FormationResolutionTests(unittest.TestCase):
    def test_tightly_spaced_number_keeps_its_formation_label(self) -> None:
        ...
        event = resolver.FormationEvent(
            ...
            signatures=[[row for _, row in frame.iterrows()]],
        )
        cluster = NumberCluster(
            observations=[frame.iloc[0]],
        )

        labels = nearby_formation_labels(cluster, event, frame)

        self.assertEqual([label[0] for label in labels], ["MAIGNAN"])
    ...
    def test_candidate_names_include_wide_players_outside_anchor_span(
        ...
        names = {str(row["text"]) for row in resolver.formation_name_rows(frame, anchors)}
    ...
    def test_number_gap_is_learned_from_current_layout(self) -> None:
        ...
        compact_gap = resolver.formation_number_gap(resolver.formation_anchor_pairs(compact))
        tall_gap = resolver.formation_number_gap(resolver.formation_anchor_pairs(tall))
